...
export default function Wrapper({ children }) {
  ...
  const [WLPrice, setWLPrice] = useState(0.005);
  const [Supply, setSupply] = useState(5000);
  const [WLSupply, setWLSupply] = useState(0);
  const [Minted, setMinted] = useState(0);
  const [MaxPerWallet, setMaxperWallet] = useState(15);
  const [MaxPerWalletWL, setMaxperWalletWL] = useState(15);
  ...
  const ContractAddress = "0x824ce596D48bA8F28b588D815e2A15bA5651FcaC";
  const ChainID = { network: "Mainnet", ID: 1 };
  ...
  const providerOptions = {
    walletconnect: {
      ...
      options: {
        infuraId: "1116a288edfe493e83f178583469195b", // required
      },
    },
  };
